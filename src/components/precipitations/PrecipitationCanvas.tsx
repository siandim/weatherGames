...
const RaindropCanvas: React.FC<RaindropCanvasProps> = ({
      ...
      if (showPuddle) {
        ...
        const puddleImage =
          temp[0] < 32 && temp[2] < 32 && temp[3] < 32
            ? precipImages["snow-puddle"] // New snow puddle image
            : precipImages["puddle"]; // Regular rain puddle
        const numPuddles = relativeHumidity[2] > 60 ? 3 : 2;
        drawPuddle(ctx, puddleImage, canvas.width, canvas.height, numPuddles); // Pass numPuddles to control the number of puddles
      }
  ...
  const drawPuddle = (
    ...
    canvasWidth: number,
    canvasHeight: number,
    numPuddles: number // Number of puddles to show
  ) => {
    const puddleWidth = 500;
    const puddleHeight = 50;
    const margin = 20;
    ...
    const puddleY = canvasHeight - puddleHeight - 5;
